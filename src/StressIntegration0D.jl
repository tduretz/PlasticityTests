...
function Vermeer90_StressIntegration_tot(σi; params=(
    ...
    law = params.law
    _type(law) = Val{law}()
    yield(τ, λ̇, ϕ, c, ηvp, law) = _yield( τ, λ̇, ϕ, c, ηvp, _type(law)) 
    _yield( τ, λ̇, ϕ, c, ηvp, ::Val{:MC_Vermeer1990}) = MohrCoulombVermeer1990_tot(τ, λ̇, ϕ, c, ηvp)
    _yield( τ, λ̇, ϕ, c, ηvp, ::Val{:DruckerPrager})  =          DruckerPrager_tot(τ, λ̇, ϕ, c, ηvp)
    ...
    for it=2:nt
        ...
        f      = yield(σ_vec, λ̇, ϕ, c, ηvp, law)
        q      = σ -> yield(σ, λ̇, ψ, c, ηvp, law)
...
_type(law) = Val{law}()
yield(τ, λ̇, ϕ, c, ηvp, law) = _yield( τ, λ̇, ϕ, c, ηvp, _type(law)) 
_yield( τ, λ̇, ϕ, c, ηvp, ::Val{:MC_Vermeer1990}) = MohrCoulombVermeer1990_vdev(τ, λ̇, ϕ, c, ηvp)
_yield( τ, λ̇, ϕ, c, ηvp, ::Val{:DruckerPrager})  =          DruckerPrager_vdev(τ, λ̇, ϕ, c, ηvp)
...
for it=2:nt
    ...
    f      = yield(τ_vec, λ̇, ϕ, c, ηvp, law)
    q      = τ -> yield(τ, λ̇, ψ, c, ηvp, law)
